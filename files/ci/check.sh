...
if [ "$(cat /tmp/comments_array.json)" != "[]" ]; then
    curl "https://github-api-bot.herokuapp.com/comments?repo=${github.repository}&pr=${github.event.number}" > /tmp/pr_comments.json
    ...
    curl -XPOST "https://github-api-bot.herokuapp.com/send_review?repo=${github.repository}&pr=${github.event.number}&body=Спасибо%20за%20PR.%20Обратите%20внимание%20на%20результаты%20автоматической%20проверки%20орфографии%20и%20ссылок" -d @/tmp/send_comments.json
